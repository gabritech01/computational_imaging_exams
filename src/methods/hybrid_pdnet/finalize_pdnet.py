...
def run():
    device = get_device()
    recon_root = os.path.join(RESULTS_DIR, "pdnet")
    ...
    for level in NOISE_LEVELS:
        ...
        model = PDNet(blur, channels=3, num_iterations=8).to(device)
        ckpt = torch.load(f"results/checkpoints/pdnet_sigma_{level}.pt", map_location=device)
            ...
            for i, (name, x, y) in enumerate(pairs):
                ...
                if i < N_EXAMPLES:
                    ...
                    for ax, img, title in zip(
                        axes, [x, y, rec], ["originale", f"degradata (sigma={level})", "ricostruita PD-Net"]
                    ):
    ...
    with open(os.path.join(RESULTS_DIR, "tables", "pdnet_summary.csv"), "w", newline="") as f:
